...
import org.pache.poi.hssf.usermodel.HSSFWorkbook;
import org.pache.poi.ss.usermodel.Row;
import org.pache.poi.ss.usermodel.Sheet;
import org.pache.poi.ss.usermodel.Workbook;
...
public class ReportDB 
{
	public static Workbook getUserEmail()
	{
		...
		row.createCell(0).setCell("The User Email report");

		// create the header row
		row = sheet.createRow(2);
		row.createCell(0).setCellValue("LsatName");
	...
	{
		// ceate the workbook, its worksheet, and its title row
		...
		var query = String.format("SELECT DownloadDate, "
			+ "Download.ProductCode, "
			+ "Email, FirstName, LastName "
			+ "From User INNER JOIN Download ON User.UserID = Download.UserID "
			+ "WHERE DownloadDate >= '%s' AND DownloadDate <= '%s' "
			+ "ORDER BY DownloadDate DESC", startDate, endDate);
